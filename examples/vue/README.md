# Frontmail example – Vue

Vite + Vue 3 contact form using the `Frontmail` plugin, `<FrontmailForm>` (with Turnstile) and the
`useSendEmail` composable (`data-testid="quick-*"`).

Source: [`frontmail-dev/frontmail-js` → `examples/vue`](https://github.com/frontmail-dev/frontmail-js/tree/main/examples/vue).
...
```sh
pnpm --filter @frontmail/example-vue dev     # dev server
pnpm --filter @frontmail/example-vue build   # production build → dist/
```
