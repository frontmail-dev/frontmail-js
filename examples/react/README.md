# Frontmail example – React

Vite + React 19 contact form using `<FrontmailProvider>`, `<FrontmailForm>` (with Turnstile) and
the `useSendEmail` hook (`data-testid="quick-*"`).

Source: [`frontmail-dev/frontmail-js` → `examples/react`](https://github.com/frontmail-dev/frontmail-js/tree/main/examples/react).
...
```sh
pnpm --filter @frontmail/example-react dev     # dev server
pnpm --filter @frontmail/example-react build   # production build → dist/
```
